...
/**
        \brief Calling this function tries to attach to the console of the
   parent process and redirect all inputs and outputs from and to this console.

        The redirected streams are stdout, stdin, stderr, cout, wcout, cin,
   wcin, wcerr, cerr, wclog and clog.
...
bool attachToConsole(/*enum ATTACH_ONLY|TRY_ATTACH_ELSE_CREATE|CREATE_NEW*/) {
  ...
  // In case that the parent process has no console return false and do no
...
#else
bool attachToConsole() { return false; }
...
/*!
    ...
    -v --verbose needs a following parameter defining the verbose level as a
   number from 0 to 3.
 */
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if (argString.startsWith("-psn_") == false) {
      ...
      //				std::cerr << std::endl << "The parameter
      //"<< i << " is an invalid finder parameter. Parameter was " << argv[i] <<
      //std::endl;
    }
  }
  ...
  try {
    ...
    // Define a value argument and add it to the command line.
    TCLAP::ValueArg<int> verboselevelArg(
        "v", "verbose",
        "Sets how many info is written to the log. 0 means with debug info, 3 "
        "means critical messages only",
        false, 1, "int");
    ...
    std::cerr << std::endl
              << "error: " << e.error() << ". " << e.argId() << std::endl;
    returnValue = 1;
    ...
    std::cerr << std::endl
              << "There was an error! Maybe faulty command line arguments set. "
                 "See --help."
              << std::endl;
    returnValue = 1;
  }
  ...
  if (returnValue != 0 || tclapExitExceptionThrown) {
    ...
    if (attachedToConsole) {
      // Workaround for skipped command line prompt: Get the current working
      // directory and print it to console.
      char *buffer;
  ...
  // Setting UTF-8 as default 8-Bit encoding to ensure that qDebug does no false
  // string conversion.
  QTextCodec::setCodecForLocale(QTextCodec::codecForName("UTF-8"));
  // Force creation of an TSLogger instance here, to avoid recursion with
  ...
  qInfo() << QString("Starting UiGUI Version %1 %2")
                 .arg(PROGRAM_VERSION_STRING)
                 .arg(PROGRAM_REVISION)
                 .toLatin1();
  qInfo() << QString("Running on %1")
  ...
  try {
    ...
    qCritical() << __LINE__ << " " << __FUNCTION__
                << ": Something went terribly wrong:" << ex.what();
  }
