...
/*!
    ...
    \brief A container for the real \a AboutDialog. Makes the 3D animation
   possible.

    The 3D animation shall suggest the user, that he is looking at his desktop,
   while this animation is done. Since this is not directly possible, \a
   AboutDialogGraphicsView when shown starts in frameless fullscreen mode with a
   screenshot of the desktop as background.
*/
...
AboutDialogGraphicsView::AboutDialogGraphicsView(AboutDialog *aboutDialog,
    ...
    : QGraphicsView(parentWindow), _aboutDialog(NULL),
      _graphicsProxyWidget(NULL), _parentWindow(NULL), _timeLine(NULL),
  ...
  QRect newGeometry =
      QRect(availableGeometry.x(), availableGeometry.y(),
            availableGeometry.width(), availableGeometry.height());
#else
  QRect newGeometry = QRect(-1, -1, QApplication::desktop()->rect().width() + 2,
                            QApplication::desktop()->rect().height() + 2);
#endif
...
/*!
    \brief Grabs a screenshot of the full desktop and shows that as background.
   Above that background the AboutDialog 3D animation is shown. Also grabs the
   content of the AboutDialog itself.
 */
void AboutDialogGraphicsView::show() {
  // Because on X11 system the window decoration is only available after a
  // widget has been shown once, we can detect _windowTitleBarWidth here for the
  // first time.
  _windowTitleBarWidth = _parentWindow->geometry().y() - _parentWindow->y();
  // If the _windowTitleBarWidth could not be determined, try it a second way.
  // Even the chances are low to get good results.
  if (_windowTitleBarWidth == 0)
    _windowTitleBarWidth = _parentWindow->frameGeometry().height() -
                           _parentWindow->geometry().height();
#ifdef Q_OS_LINUX
  if (_windowTitleBarWidth == 0) {
    // TODO: 27 pixel is a fix value for the Ubuntu 10.4 default window theme
    // and so just a workaround for that specific case.
    _windowPosOffset = 27;
  ...
  _graphicsProxyWidget->setPos(_parentWindow->geometry().x() +
                                   (_parentWindow->geometry().width() -
                                    _graphicsProxyWidget->geometry().width()) /
                                       2,
                               _parentWindow->y() + _windowTitleBarWidth -
                                   _windowPosOffset);

  QRectF r = _graphicsProxyWidget->boundingRect();
  _graphicsProxyWidget->setTransform(
      QTransform()
...
/*!
    \brief Stops the 3D animation, moves the AboutDialog to the correct place
   and really shows it.
 */
...
/*!
    \brief Does not directly hide the AboutDialog but instead starts the "fade
   out" 3D animation.
 */
