...
/*!
    ...
    \brief SettingsPaths is a pure static functions class from which info about
   the paths needed for settings can be retrieved.
*/
...
/*!
    ...
    Mainly during this init it is detected whether to start in portable mode or
   not. This is done by testing whether the directory "config" is in the same
   directory as this applications executable file. In portable mode all data is
   ONLY written to subdirectories of the applications executable file. Means
   also that the directory "indenters" has to be there. In not portable mode
   (multiuser mode) only users home directory is used for writing config data.
 */
void SettingsPaths::init() {
  _alreadyInitialized = true;

  qDebug() << __LINE__ << " " << __FUNCTION__
           << ": Initializing application paths.";

  // Get the applications binary path, with respect to MacOSXs use of the .app
  // folder.
  ...
  if (indexOfDotApp != -1) {
    ...
    // Cut off after the first slash that was in front of ".app" (normally this
    // is the word "UniversalIndentGUI")
    _applicationBinaryPath =
        _applicationBinaryPath.left(_applicationBinaryPath.lastIndexOf("/"));
  }
#endif

  // If the "config" directory is a subdir of the applications binary path, use
  // this one (portable mode)
  ...
  if (QFile::exists(_settingsPath)) {
    ...
    // TODO: If the portable drive has write protection, use local temp path and
    // clean it up on exit.
  ...
  // ... otherwise use the system specific global application data path.
    ...
    // On windows systems the directories "indenters", "translations" are
    // subdirs of the _applicationBinaryPath.
    _globalFilesPath = _applicationBinaryPath;
#else
    // Remove any trailing slashes.
    ...
    if (!QFile::exists(_settingsPath + "/UiGuiSyntaxHighlightConfig.ini")) {
      QFile::copy(_globalFilesPath + "/config/UiGuiSyntaxHighlightConfig.ini",
                  _settingsPath + "/UiGuiSyntaxHighlightConfig.ini");
    }
    _indenterPath = _globalFilesPath + "/indenters";

    // On different systems it may be that "QDir::tempPath()" ends with a "/" or
    // not. So check this. Remove any trailing slashes.
  ...
  qDebug() << __LINE__ << " " << __FUNCTION__
           ...
           << "</li><li>Running in portable mode=" << _portableMode
           << "</li></ul>";
}
...
/*!
    \brief Returns the path where the files concerning all users reside. For
...
/*!
    \brief Returns the path where the where all temporary data should be written
   to.
 */
...
/*!
    \brief Completely deletes the created temporary directory with all of its
   content.
 */
void SettingsPaths::cleanAndRemoveTempDir() {
  ...
  while (dirIterator.hasNext()) {
    ...
    // If this dummy call isn't done here, calling
    // "dirIterator.fileInfo().isDir()" later somehow fails.
    dirIterator.fileInfo();

    if (!currentDirOrFile.isEmpty() && dirIterator.fileName() != "." &&
        dirIterator.fileName() != "..") {
      // There is a path on the stack but the current path doesn't start with
      // that path. So we changed into another parent directory and the one on
      // the stack can be deleted since it must be empty.
      if (!directoryStack.isEmpty() &&
          !currentDirOrFile.startsWith(directoryStack.top())) {
        ...
        if (couldRemoveDir == false)
          qWarning() << __LINE__ << " " << __FUNCTION__
                     << "Could not remove the directory: " << dirToBeRemoved;
      ...
      // otherwise it must be a file, so delete it.
        ...
        if (couldRemoveFile == false)
          qWarning() << __LINE__ << " " << __FUNCTION__
                     << "Could not remove the file: " << currentDirOrFile;
