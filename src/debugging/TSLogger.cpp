...
/*!
    \class TSLogger
    \brief This class handles any kind of data logging, for debugging and
   whatever purpose.

    Beneath being able of displaying a dialog window containing all log messages
   of the current session, a log file in the systems temporary directory is
   used. Its name is "UiGUI_log.html".

    Setting a verbose level allows to only write messages that have the selected
   minimum priority to the log.
 */

/*!
    \brief Returns the only existing instance of TSLogger. If the instance
...
/*!
    \brief Initializes the dialog and sets the path to the log file in the
   systems temporary directory. Sets the default verbose level to warning level.
 */
TSLogger::TSLogger(int verboseLevel) : QDialog() {
  ...
  connect(_TSLoggerDialogForm->openLogFileFolderToolButton, SIGNAL(clicked()),
          this, SLOT(openLogFileFolder()));
...
/*!
    \brief Logs all incoming messages \a msg to the dialogs text edit and to the
   log file.

    Only messages whos \a type have a higher priority than the set verbose level
   are logged.
 */
void TSLogger::messageHandler(QtMsgType type, const QMessageLogContext &,
                              const QString &msg) {
  if (_instance == NULL)
  ...
  */
  ...
  // Only log messages that have a higher or equal priority than set with the
  // verbose level.
  ...
  // Depending on the QtMsgType prepend a different colored Debug, Warning,
  // Critical or Fatal.
  switch (type) {
  case QtDebugMsg:
  ...
  case QtWarningMsg:
    message +=
        " <span style=\"font-weight:bold; color:gold;\">Warning:</span> ";
    break;
  case QtCriticalMsg:
  ...
  case QtFatalMsg:
    message +=
        " <span style=\"font-weight:bold; color:#D60000;\">Fatal:</span> ";
  }
...
/*!
    \brief Opens the folder that contains the created log file with the name
   "UiGUI_log.html".
 */
...
void TSLogger::writeToLogFile(const QString &message) {
  // If the file where all logging messages should go to isn't initilized yet,
  // do that now.
  if (_logFileInitState == NOTINITIALZED) {
    ...
    // To make the temporary log file invulnerable against file symbolic link
    // hacks append the current date and time up to milliseconds to its name and
    // a random character.
    QString logFileName =
        "UiGUI_log_" + QDateTime::currentDateTime().toString("yyyyMMdd");
    logFileName += "-" + QDateTime::currentDateTime().toString("hhmmsszzz");
    // By random decide whether to append a number or an upper or lower case
    // character.
    qsrand(time(NULL));
    ...
    // Set the tooltip of the open log file folder button to show the unique
    // name of the log file.
    _TSLoggerDialogForm->openLogFileFolderToolButton->setToolTip(
        _TSLoggerDialogForm->openLogFileFolderToolButton->toolTip() + " (" +
        logFileName + ")");
  ...
  // If the logging file is initialzed, write all messages contained in the
  // message queue into the file.
