...
template <typename T> inline void UNUSED_PARAMETER_WARNING_AVOID(T) {}
...
/*!
    ...
    \brief A widget for handling many indenters that are configured by an ini
   file.

    This is a widget that is used by the main window. It handles access to the
    indenter config file and calls the chosen indenter to reformat the source
   text. Calls the indenter each time a setting has been changed and informs the
   main window about the reformatted source code.
*/

/*!
    \brief Constructor of the indent handler.

    By calling this constructor the indenter to be loaded, can be selected by
   setting its \a indenterID, which is the number of found indenter ini files in
   alphabetic order starting at index 0.
 */
IndentHandler::IndentHandler(int indenterID, QWidget *mainWindow,
                             QWidget *parent)
    : QWidget(parent), _indenterSelectionCombobox(NULL),
      _indenterParameterHelpButton(NULL), _toolBoxContainerLayout(NULL),
      _indenterParameterCategoriesToolBox(NULL), _indenterSettings(NULL),
      _mainWindow(NULL), _errorMessageDialog(NULL), _menuIndenter(NULL),
  ...
  Q_ASSERT_X(indenterID >= 0, "IndentHandler",
             "the selected indenterID is < 0");
  ...
  connect(_actionLoadIndenterConfigFile, SIGNAL(triggered()), this,
          SLOT(openConfigFileDialog()));
  connect(_actionSaveIndenterConfigFile, SIGNAL(triggered()), this,
          SLOT(saveasIndentCfgFileDialog()));
  ...
  _indenterSelectionCombobox->setSizeAdjustPolicy(
      QComboBox::AdjustToMinimumContentsLengthWithIcon);
  _indenterSelectionCombobox->setMinimumContentsLength(20);
  connect(_indenterSelectionCombobox, SIGNAL(activated(int)), this,
          SLOT(setIndenter(int)));
  UiGuiSettings::getInstance()->registerObjectProperty(
      _indenterSelectionCombobox, "currentIndex", "selectedIndenter");
  ...
  _indenterParameterHelpButton->setObjectName(
      QString::fromUtf8("indenterParameterHelpButton"));
  _indenterParameterHelpButton->setIcon(
      QIcon(QString::fromUtf8(":/mainWindow/help.png")));
  hboxLayout->addWidget(_indenterParameterHelpButton);
  // Handle if the indenter parameter help button is pressed.
  connect(_indenterParameterHelpButton, SIGNAL(clicked()), this,
          SLOT(showIndenterManual()));

  // create a toolbox and set its resize behavior
  _indenterParameterCategoriesToolBox = new QToolBox(this);
  _indenterParameterCategoriesToolBox->setObjectName(
      QString::fromUtf8("_indenterParameterCategoriesToolBox"));

#ifdef UNIVERSALINDENTGUI_NPP_EXPORTS
  connect(_indenterParameterCategoriesToolBox, SIGNAL(currentChanged(int)),
          this, SLOT(updateDrawing()));
#endif // UNIVERSALINDENTGUI_NPP_EXPORTS

  //_indenterParameterCategoriesToolBox->setSizePolicy( QSizePolicy::Expanding,
  //QSizePolicy::Expanding );
  //_indenterParameterCategoriesToolBox->setMaximumSize(QSize(16777215,
  //16777215));
  ...
  if (_indenterIniFileList.count() > 0) {
    // Take care if the selected indenterID is smaller or greater than the
    // number of existing indenters
    if (indenterID < 0) {
      indenterID = 0;
    }
    ...
    // Reads and parses the by indenterID defined indent ini file and creates
    // toolbox entries
    readIndentIniFile(_indenterDirctoryStr + "/" +
                      _indenterIniFileList.at(indenterID));
    ...
    // Fill the indenter selection combo box with the list of available
    // indenters.
    if (!getAvailableIndenters().isEmpty()) {
      ...
      connect(_indenterSelectionCombobox, SIGNAL(currentIndexChanged(int)),
              this, SIGNAL(selectedIndenterIndexChanged(int)));
    }
  } else {
    _errorMessageDialog->showMessage(
        tr("No indenter ini files"),
        tr("There exists no indenter ini files in the directory \"") +
            QDir(_indenterDirctoryStr).absolutePath() + "\".");
  }
...
/*!
    \brief Implicitly writes the current indenter parameters to the indenters
   config file.
 */
IndentHandler::~IndentHandler() {
  // Generate the parameter string that will be saved to the indenters config
  // file.
  QString parameterString = getParameterString();
  if (!_indenterFileName.isEmpty()) {
    saveConfigFile(_settingsDirctoryStr + "/" + _indenterFileName + ".cfg",
                   parameterString);
  }
...
/*!
    \brief Initializes the context menu used for some actions like saving the
   indenter config file.
 */
void IndentHandler::initIndenterMenu() {
  if (_menuIndenter == NULL) {
    _actionLoadIndenterConfigFile = new QAction(this);
    _actionLoadIndenterConfigFile->setObjectName(
        QString::fromUtf8("_actionLoadIndenterConfigFile"));
    _actionLoadIndenterConfigFile->setIcon(
        QIcon(QString::fromUtf8(":/mainWindow/load_indent_cfg.png")));

    _actionSaveIndenterConfigFile = new QAction(this);
    _actionSaveIndenterConfigFile->setObjectName(
        QString::fromUtf8("_actionSaveIndenterConfigFile"));
    _actionSaveIndenterConfigFile->setIcon(
        QIcon(QString::fromUtf8(":/mainWindow/save_indent_cfg.png")));

    _actionCreateShellScript = new QAction(this);
    _actionCreateShellScript->setObjectName(
        QString::fromUtf8("_actionCreateShellScript"));
    _actionCreateShellScript->setIcon(
        QIcon(QString::fromUtf8(":/mainWindow/shell.png")));

    _actionResetIndenterParameters = new QAction(this);
    _actionResetIndenterParameters->setObjectName(
        QString::fromUtf8("_actionResetIndenterParameters"));
    _actionResetIndenterParameters->setIcon(
        QIcon(QString::fromUtf8(":/mainWindow/view-refresh.png")));
...
/*!
    \brief Returns the context menu used for some actions like saving the
   indenter config file.
 */
QMenu *IndentHandler::getIndenterMenu() { return _menuIndenter; }

/*!
    \brief Returns the actions of the context menu used for some actions like
   saving the indenter config file.
 */
...
/*!
    \brief Opens the context menu, used for some actions like saving the
   indenter config file, at the event position.
 */
...
/*!
    \brief Creates the content for a shell script that can be used as a external
   tool call to indent an as parameter defined file.
 */
QString IndentHandler::generateShellScript(const QString &configFilename) {
  ...
  // Define the placeholder for parameter variables either in batch or bash
  // programming.
  ...
  parameterInputFile =
      " " + _inputFileParameter + "\"" + shellParameterPlaceholder + "\"";

  if (_outputFileParameter != "none" && _outputFileParameter != "stdout") {
    if (_outputFileName == _inputFileName) {
      parameterOuputFile =
          " " + _outputFileParameter + "\"" + shellParameterPlaceholder + "\"";
    } else {
      parameterOuputFile =
          " " + _outputFileParameter + _outputFileName + ".tmp";
    }
  }

  // If the config file name is empty it is assumed that all parameters are sent
  // via command line call
  if (_globalConfigFilename.isEmpty()) {
    parameterParameterFile = " " + getParameterString();
  }
  // else if needed add the parameter to the indenter call string where the
  // config file can be found.
  else if (_useCfgFileParameter != "none") {
    parameterParameterFile =
        " " + _useCfgFileParameter + "\"./" + configFilename + "\"";
  }

  // Assemble indenter call string for parameters according to the set order.
  if (_parameterOrder == "ipo") {
    indenterCompleteCallString =
        parameterInputFile + parameterParameterFile + parameterOuputFile;
  } else if (_parameterOrder == "pio") {
    indenterCompleteCallString =
        parameterParameterFile + parameterInputFile + parameterOuputFile;
  } else if (_parameterOrder == "poi") {
    indenterCompleteCallString =
        parameterParameterFile + parameterOuputFile + parameterInputFile;
  } else {
    indenterCompleteCallString =
        parameterInputFile + parameterOuputFile + parameterParameterFile;
  }
  ...
  if (_outputFileParameter == "stdout") {
    indenterCompleteCallString =
        indenterCompleteCallString + " >" + _outputFileName + ".tmp";
  }

  // If the output filename is not the same as the input filename copy the
  // output over the input.
  if (_outputFileName != _inputFileName) {
#if defined(Q_OS_WIN32)
    replaceInputFileCommand = "move /Y " + _outputFileName + ".tmp \"" +
                              shellParameterPlaceholder + "\"\n";
#else
    replaceInputFileCommand = "mv " + _outputFileName + ".tmp \"" +
                              shellParameterPlaceholder + "\"\n";
#endif
  }
  ...
  shellScript = shellScript.replace("__INDENTERCALLSTRING2__",
                                    indenterCompleteCallString + "\n" +
                                        replaceInputFileCommand);
  indenterCompleteCallString =
      indenterCompleteCallString.replace("$1", "$file2indent");
  replaceInputFileCommand =
      replaceInputFileCommand.replace("$1", "$file2indent");
  shellScript = shellScript.replace("__INDENTERCALLSTRING1__",
                                    indenterCompleteCallString + "\n" +
                                        replaceInputFileCommand);
#endif
...
/*!
    ...
    The \a inputFileExtension has to be given as parameter so the called
   indenter can identify the programming language if needed.
 */
...
/*!
    \brief Format \a sourceCode by calling the interpreted JavaScript code of
   the indenter.
...
/*!
    \brief Format \a sourceCode by calling the binary executable of the
   indenter.
...
QString IndentHandler::callExecutableIndenter(QString sourceCode,
                                              QString inputFileExtension) {
  Q_ASSERT_X(!_inputFileName.isEmpty(), "callIndenter",
             "_inputFileName is empty");
  //    Q_ASSERT_X( !_outputFileName.isEmpty(), "callIndenter", "_outputFileName
  //    is empty" );
  Q_ASSERT_X(!_indenterFileName.isEmpty(), "callIndenter",
             "_indenterFileName is empty");
  ...
  if (!_globalConfigFilename.isEmpty()) {
    saveConfigFile(_tempDirctoryStr + "/" + _globalConfigFilename,
                   parameterString);
  }
  ...
  QFile inputSrcFile(_tempDirctoryStr + "/" + _inputFileName +
                     inputFileExtension);
  ...
  if (inputSrcFile.open(QFile::ReadWrite | QFile::Text)) {
    ...
    qDebug() << __LINE__ << " " << __FUNCTION__
             << ": Wrote to be indented source code to file "
             << inputSrcFile.fileName();
  } else {
    qCritical() << __LINE__ << " " << __FUNCTION__
                << ": Couldn't write to be indented source code to file "
                << inputSrcFile.fileName();
  }
  ...
  if (_inputFileParameter.trimmed() == "<" || _inputFileParameter == "stdin") {
    ...
    parameterInputFile =
        " " + _inputFileParameter + _inputFileName + inputFileExtension;
  }

  // Set the output file for the to be called indenter.
  if (_outputFileParameter != "none" && _outputFileParameter != "stdout") {
    parameterOuputFile =
        " " + _outputFileParameter + _outputFileName + inputFileExtension;
  }

#ifdef Q_OS_WIN32
  // Paths may contain Unicode or other foreign characters. Windows commands
  // line tools will receive als falsely encoded path string by QProcess or they
  // connot correctly handle the Unicode path on their own. Because of this the
  // path gets converted to Windows short paths using the 8.3 notation.

  qDebug() << __LINE__ << " " << __FUNCTION__
           << ": Temp dir before trying to convert it to short Windows path is"
           << _tempDirctoryStr;

  // At first convert the temp path to Windows like separators.
  QString tempDirctoryStrHelper =
      QDir::toNativeSeparators(_tempDirctoryStr).replace("\\", "\\\\");
  ...
  // If the short path could be retrieved, create a correct sized buffer, store
  // the short path in it and convert all back to QString.
  if (length != 0) {
#ifdef UNICODE
...
#endif
    _tempDirctoryStr =
        QDir::fromNativeSeparators(tempDirctoryStrHelper).replace("//", "/");
    delete[] buffer;

    // Check whether the short path still contains some kind of non ascii
    // characters.
    if (_tempDirctoryStr.length() != _tempDirctoryStr.toLatin1().length()) {
      qWarning() << __LINE__ << " " << __FUNCTION__
                 << ": Shortened path still contains non ascii characters. "
                    "Could cause some indenters not to work properly!";
    }
  } else {
    qWarning() << __LINE__ << " " << __FUNCTION__
  ...
  qDebug() << __LINE__ << " " << __FUNCTION__
           << ": Temp dir after trying to convert it to short Windows path is "
           << _tempDirctoryStr;
  ...
  // if needed add the parameter to the indenter call string where the config
  // file can be found
  else if (_useCfgFileParameter != "none") {
    parameterParameterFile = " " + _useCfgFileParameter + "\"" +
                             _tempDirctoryStr + "/" + _globalConfigFilename +
                             "\"";
  }
  ...
  // If no indenter executable call string could be created before, show an
  // error message.
  if (_indenterExecutableCallString.isEmpty()) {
    _errorMessageDialog->showMessage(
        tr("No indenter executable"),
        tr("There exists no indenter executable with the name \"%1\" in the "
           "directory \"%2\" nor in the global environment.")
            .arg(_indenterFileName)
            .arg(_indenterDirctoryStr));
    return sourceCode;
  }
  ...
  // Set the directory where the indenter will be executed for the process'
  // environment as PWD.
  ...
  indentProcess.setWorkingDirectory(
      QFileInfo(_tempDirctoryStr).absoluteFilePath());

  qDebug() << __LINE__ << " " << __FUNCTION__
           << ": Will call the indenter in the directory "
           << indentProcess.workingDirectory()
  ...
  if (!calledProcessSuccessfully) {
    ...
    processReturnString += tr("<b>Returned error message:</b> ") +
                           indentProcess.errorString() + "<br>";

    switch (indentProcess.error()) {
    case QProcess::FailedToStart:
      processReturnString +=
          tr("<b>Reason could be:</b> ") +
          "The process failed to start. Either the invoked program is missing, "
          "or you may have insufficient permissions to invoke the program.<br>";
      break;
    case QProcess::Crashed:
      processReturnString +=
          "The process crashed some time after starting successfully.<br>";
      break;
    case QProcess::Timedout:
      processReturnString += "The called indenter did not response for over 10 "
                             "seconds, so aborted its execution.<br>";
      break;
    case QProcess::WriteError:
      processReturnString +=
          "An error occurred when attempting to write to the process. For "
          "example, the process may not be running, or it may have closed its "
          "input channel.<br>";
      break;
    case QProcess::ReadError:
      processReturnString +=
          "An error occurred when attempting to read from the process. For "
          "example, the process may not be running.<br>";
      break;
    case QProcess::UnknownError:
      processReturnString += "An unknown error occurred. This is the default "
                             "return value of error().<br>";
      break;
    default:
      break;
    }
    processReturnString += tr("<br><b>Callstring was:</b> ") +
                           encodeToHTML(indenterCompleteCallString);
    processReturnString +=
        tr("<br><br><b>Indenter output was:</b><pre>") + "<br>" +
    ...
    _errorMessageDialog->showMessage(tr("Error calling Indenter"),
                                     processReturnString);
  }
  ...
  if (indentProcess.exitCode() != 0) {
    ...
    processReturnString =
        tr("<b>Indenter returned with exit code:</b> ") + exitCode + "<br>" +
        tr("<b>Indent console output was:</b> ") + "<br>" +
        "(STDOUT):" + encodeToHTML(indentProcess.readAllStandardOutput()) +
        "<br>" +
        "(STDERR):" + encodeToHTML(indentProcess.readAllStandardError()) +
        "<br>" + tr("<br><b>Callstring was:</b> ") +
        encodeToHTML(indenterCompleteCallString) + "</html></body>";
    qWarning() << __LINE__ << " " << __FUNCTION__ << processReturnString;
    QApplication::restoreOverrideCursor();
    _errorMessageDialog->showMessage(tr("Indenter returned error"),
                                     processReturnString);
  }
  ...
  if (calledProcessSuccessfully) {
    ...
    if (indentProcess.exitCode() == 0 && _outputFileParameter == "stdout") {
      ...
      qDebug() << __LINE__ << " " << __FUNCTION__
               << ": Read indenter output from StdOut.";
    }
    // ... else read the output file generated by the indenter call.
    else {
      QFile outSrcFile(_tempDirctoryStr + "/" + _outputFileName +
                       inputFileExtension);
      if (outSrcFile.open(QFile::ReadOnly | QFile::Text)) {
        ...
        outSrcFile.close();
        qDebug() << __LINE__ << " " << __FUNCTION__
                 << ": Read indenter output from file "
                 << outSrcFile.fileName();
      } else {
        qCritical() << __LINE__ << " " << __FUNCTION__
                    << ": Couldn't read indenter output from file "
                    << outSrcFile.fileName();
      }
    }
  } else {
...
/*!
    \brief Generates and returns a string with all parameters needed to call the
   indenter.
 */
QString IndentHandler::getParameterString() {
  ...
  foreach (ParamNumeric pNumeric, _paramNumerics) {
    if (pNumeric.valueEnabledChkBox->isChecked()) {
      parameterString += pNumeric.paramCallName +
                         QString::number(pNumeric.spinBox->value()) +
  ...
  foreach (ParamString pString, _paramStrings) {
    if (!pString.lineEdit->text().isEmpty() &&
        pString.valueEnabledChkBox->isChecked()) {
      // Create parameter definition for each value devided by a | sign.
      foreach (QString paramValue, pString.lineEdit->text().split("|")) {
        parameterString +=
            pString.paramCallName + paramValue + _cfgFileParameterEnding;
      }
    }
  }
  ...
  foreach (ParamMultiple pMultiple, _paramMultiples) {
    if (pMultiple.valueEnabledChkBox->isChecked()) {
      parameterString +=
          pMultiple.choicesStrings.at(pMultiple.comboBox->currentIndex()) +
          _cfgFileParameterEnding;
    }
  }
...
/*!
    \brief Load the config file for the indenter and apply the settings made
   there.
 */
bool IndentHandler::loadConfigFile(QString filePathName) {
  ...
  // If the to be loaded config file does not exist leave all values as they are
  // and return false.
  if (!cfgFile.exists()) {
    return false;
  }
  ...
  foreach (ParamBoolean pBoolean, _paramBooleans) {
    ...
    if (pBoolean.trueString.length() > pBoolean.falseString.length()) {
      ...
      // if true string not found, search for false string
        ...
        index =
            cfgFileData.indexOf(pBoolean.falseString, 0, Qt::CaseInsensitive);
        ...
        // neither true nor false parameter found so use default value
          ...
          paramValue =
              _indenterSettings->value(pBoolean.paramName + "/ValueDefault")
                  .toBool();
        }
      }
    }
    // the false parameter string is longer than the true string
      ...
      // if false string not found, search for true string
        ...
        index =
            cfgFileData.indexOf(pBoolean.trueString, 0, Qt::CaseInsensitive);
        // if true string found set the parameter value to true
        if (index != -1) {
          paramValue = true;
        }
  ...
  // Search for name of each numeric parameter and set the value found behind
  // it.
  foreach (ParamNumeric pNumeric, _paramNumerics) {
    ...
    if (index != -1) {
      ...
      // Find the end of the parameter by searching for set config file
      // parameter ending. Most of time this is a carriage return.
      ...
      // disable the signal-slot connection. Otherwise signal is emmitted each
      // time when value is set
      QObject::disconnect(pNumeric.spinBox, SIGNAL(valueChanged(int)), this,
                          SLOT(handleChangedIndenterSettings()));
      pNumeric.spinBox->setValue(paramValue);
      pNumeric.valueEnabledChkBox->setChecked(true);
      QObject::connect(pNumeric.spinBox, SIGNAL(valueChanged(int)), this,
                       SLOT(handleChangedIndenterSettings()));
    }
    // parameter was not found in config file
    else {
      int defaultValue =
          _indenterSettings->value(pNumeric.paramName + "/ValueDefault")
              .toInt();
  ...
  foreach (ParamString pString, _paramStrings) {
    ...
    if (index != -1) {
      while (index != -1) {
        ...
        // Set index after the parameter name, so it points to the front of the
        // string value.
        ...
        // If the same parameter has been set multiple times, concatenate the
        // strings dvivided by a |.
        else {
          paramValueStr = paramValueStr + "|" +
                          QString(cfgFileData.mid(index, crPos - index));
        }
    ...
    // Parameter was not found in config file
      ...
      paramValueStr =
          _indenterSettings->value(pString.paramName + "/ValueDefault")
              .toString();
      pString.lineEdit->setText(paramValueStr);
  ...
  foreach (ParamMultiple pMultiple, _paramMultiples) {
    ...
    if (index == -1) {
      int defaultValue =
          _indenterSettings->value(pMultiple.paramName + "/ValueDefault")
              .toInt();
...
/*!
    \brief Sets all indenter parameters to their default values defined in the
   ini file.
 */
void IndentHandler::resetToDefaultValues() {
  ...
  foreach (ParamBoolean pBoolean, _paramBooleans) {
    // Boolean value that will be assigned to the checkbox.
    bool defaultValue =
        _indenterSettings->value(pBoolean.paramName + "/ValueDefault").toBool();
  ...
  foreach (ParamNumeric pNumeric, _paramNumerics) {
    int defaultValue =
        _indenterSettings->value(pNumeric.paramName + "/ValueDefault").toInt();
    pNumeric.spinBox->setValue(defaultValue);
    pNumeric.valueEnabledChkBox->setChecked(
        _indenterSettings->value(pNumeric.paramName + "/Enabled").toBool());
  }
  ...
  foreach (ParamString pString, _paramStrings) {
    ...
    pString.valueEnabledChkBox->setChecked(
        _indenterSettings->value(pString.paramName + "/Enabled").toBool());
  }

  // Search for name of each multiple choice parameter and set it.
  foreach (ParamMultiple pMultiple, _paramMultiples) {
    int defaultValue =
        _indenterSettings->value(pMultiple.paramName + "/ValueDefault").toInt();
    pMultiple.comboBox->setCurrentIndex(defaultValue);
    pMultiple.valueEnabledChkBox->setChecked(
        _indenterSettings->value(pMultiple.paramName + "/Enabled").toBool());
  }
}

/*!
    \brief Opens and parses the indenter ini file that is declared by \a
   iniFilePath.
 */
void IndentHandler::readIndentIniFile(QString iniFilePath) {
  Q_ASSERT_X(!iniFilePath.isEmpty(), "readIndentIniFile",
             "iniFilePath is empty");

  // open the ini-file that contains all available indenter settings with their
  // additional infos
  ...
  _indenterFileName =
      _indenterSettings->value("header/indenterFileName").toString();
  _globalConfigFilename =
      _indenterSettings->value("header/configFilename").toString();
  _useCfgFileParameter =
      _indenterSettings->value("header/useCfgFileParameter").toString();
  _cfgFileParameterEnding =
      _indenterSettings->value("header/cfgFileParameterEnding").toString();
  if (_cfgFileParameterEnding == "cr") {
    _cfgFileParameterEnding = "\n";
  }
  _indenterShowHelpParameter =
      _indenterSettings->value("header/showHelpParameter").toString();

  if (_indenterFileName.isEmpty()) {
    _errorMessageDialog->showMessage(
        tr("Indenter ini file header error"),
        tr("The loaded indenter ini file \"%1\"has a faulty header. At least "
           "the indenters file name is not set.")
            .arg(iniFilePath));
  }

  // Read the parameter order. Possible values are (p=parameter[file]
  // i=inputfile o=outputfile) pio, ipo, iop
  _parameterOrder =
      _indenterSettings->value("header/parameterOrder", "pio").toString();
  _inputFileParameter =
      _indenterSettings->value("header/inputFileParameter").toString();
  _inputFileName = _indenterSettings->value("header/inputFileName").toString();
  _outputFileParameter =
      _indenterSettings->value("header/outputFileParameter").toString();
  _outputFileName =
      _indenterSettings->value("header/outputFileName").toString();
  ...
  QString categoriesStr =
      _indenterSettings->value("header/categories").toString();
  categories = categoriesStr.split("|");
  // Assure that the category list is never empty. At least contain a "general"
  // section.
  ...
  // create a page for each category and store its references in a
  // toolboxpage-array
  foreach (QString category, categories) {
    categoryPage.widget = new QWidget();
    categoryPage.widget->setObjectName(category);
    categoryPage.widget->setSizePolicy(QSizePolicy::MinimumExpanding,
  ...
  foreach (QString indenterParameter, _indenterParameters) {
    // if it is not the indent header definition read the parameter and add it
    // to the corresponding category toolbox page
    if (indenterParameter != "header") {
      // read to which category the parameter belongs
      int category =
          _indenterSettings->value(indenterParameter + "/Category").toInt();
      // Assure that the category number is never greater than the available
      // categories.
      ...
      QString editType =
          _indenterSettings->value(indenterParameter + "/EditorType")
              .toString();

      // edit type is numeric so create a spinbox with label
      if (editType == "numeric") {
        // read the parameter name as it is used at the command line or in its
        // config file
        QString parameterCallName =
            _indenterSettings->value(indenterParameter + "/CallName")
                .toString();

        // create checkbox which enables or disables the parameter
        QCheckBox *chkBox =
            new QCheckBox(_indenterParameterCategoryPages.at(category).widget);
        chkBox->setChecked(
            _indenterSettings->value(indenterParameter + "/Enabled").toBool());
        chkBox->setToolTip("Enables/disables the parameter. If disabled the "
                           "indenters default value will be used.");
        ...
        // create the spinbox
        QSpinBox *spinBox =
            new QSpinBox(_indenterParameterCategoryPages.at(category).widget);
        paramToolTip =
            _indenterSettings->value(indenterParameter + "/Description")
                .toString();
        ...
        if (_indenterSettings->value(indenterParameter + "/MinVal")
                .toString() != "") {
          spinBox->setMinimum(
              _indenterSettings->value(indenterParameter + "/MinVal").toInt());
        } else {
          spinBox->setMinimum(0);
        }
        if (_indenterSettings->value(indenterParameter + "/MaxVal")
                .toString() != "") {
          spinBox->setMaximum(
              _indenterSettings->value(indenterParameter + "/MaxVal").toInt());
        } else {
        ...
        // create the label
        QLabel *label =
            new QLabel(_indenterParameterCategoryPages.at(category).widget);
        ...
        _indenterParameterCategoryPages.at(category).vboxLayout->addLayout(
            hboxLayout);
        ...
        paramNumeric.spinBox->setValue(
            _indenterSettings->value(paramNumeric.paramName + "/ValueDefault")
                .toInt());
        _paramNumerics.append(paramNumeric);

        QObject::connect(spinBox, SIGNAL(valueChanged(int)), this,
                         SLOT(handleChangedIndenterSettings()));
        QObject::connect(chkBox, SIGNAL(clicked()), this,
                         SLOT(handleChangedIndenterSettings()));
#ifdef UNIVERSALINDENTGUI_NPP_EXPORTS
      ...
      // edit type is boolean so create a checkbox
        ...
        _indenterParameterCategoryPages.at(category).vboxLayout->addWidget(
            chkBox);
        ...
        QStringList trueFalseStrings =
            _indenterSettings->value(indenterParameter + "/TrueFalse")
                .toString()
                .split("|");
        paramBoolean.trueString = trueFalseStrings.at(0);
        paramBoolean.falseString = trueFalseStrings.at(1);
        paramBoolean.checkBox->setChecked(
            _indenterSettings->value(paramBoolean.paramName + "/ValueDefault")
                .toBool());
      ...
      // edit type is numeric so create a line edit with label
        ...
        QLineEdit *lineEdit =
            new QLineEdit(_indenterParameterCategoryPages.at(category).widget);
        paramToolTip =
            _indenterSettings->value(indenterParameter + "/Description")
                .toString();
        ...
        paramString.lineEdit->setText(
            _indenterSettings->value(paramString.paramName + "/ValueDefault")
                .toString());
        _paramStrings.append(paramString);

        QObject::connect(lineEdit, SIGNAL(editingFinished()), this,
                         SLOT(handleChangedIndenterSettings()));
        QObject::connect(chkBox, SIGNAL(clicked()), this,
                         SLOT(handleChangedIndenterSettings()));
#ifdef UNIVERSALINDENTGUI_NPP_EXPORTS
      ...
      // edit type is multiple so create a combobox with label
        ...
        QComboBox *comboBox =
            new QComboBox(_indenterParameterCategoryPages.at(category).widget);
        QStringList choicesStrings =
            _indenterSettings->value(indenterParameter + "/Choices")
                .toString()
                .split("|");
        QStringList choicesStringsReadable =
            _indenterSettings->value(indenterParameter + "/ChoicesReadable")
                .toString()
        ...
        paramMultiple.comboBox->setCurrentIndex(
            _indenterSettings->value(paramMultiple.paramName + "/ValueDefault")
                .toInt());
        _paramMultiples.append(paramMultiple);

        QObject::connect(comboBox, SIGNAL(activated(int)), this,
                         SLOT(handleChangedIndenterSettings()));
        QObject::connect(chkBox, SIGNAL(clicked()), this,
                         SLOT(handleChangedIndenterSettings()));
#ifdef UNIVERSALINDENTGUI_NPP_EXPORTS
  ...
  foreach (IndenterParameterCategoryPage categoryPage,
...
/*!
    ...
    Opens all uigui ini files found in the list \a _indenterIniFileList, opens
   each ini file and reads the there defined real name of the indenter. These
   names are being returned as QStringList.
 */
QStringList IndentHandler::getAvailableIndenters() {
  ...
  foreach (QString indenterIniFile, _indenterIniFileList) {
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
      int index = -1;
      QByteArray line;
      // Search for the string "indenterName=" and get the following string
      // until line end.
...
/*!
    \brief Deletes all elements in the toolbox and initializes the indenter
   selected by \a indenterID.
 */
void IndentHandler::setIndenter(int indenterID) {
  QApplication::setOverrideCursor(Qt::WaitCursor);

#ifdef UNIVERSALINDENTGUI_NPP_EXPORTS
  disconnect(_indenterParameterCategoriesToolBox, SIGNAL(currentChanged(int)),
             this, SLOT(updateDrawing()));
#endif // UNIVERSALINDENTGUI_NPP_EXPORTS
  ...
  // Take care if the selected indenterID is smaller or greater than the number
  // of existing indenters
  if (indenterID < 0) {
    indenterID = 0;
  }
  if (indenterID >= _indenterIniFileList.count()) {
  ...
  connect(_indenterParameterCategoriesToolBox, SIGNAL(currentChanged(int)),
          this, SLOT(updateDrawing()));
  _indenterParameterCategoriesToolBox->removeItem(
      _indenterParameterCategoriesToolBox->indexOf(&dummyWidget));
#endif // UNIVERSALINDENTGUI_NPP_EXPORTS
}

/*!
    \brief Returns a string containing by the indenter supported file
   types/extensions divided by a space.
 */
...
/*!
    \brief Tries to create a call path string for the indenter executable. If
...
bool IndentHandler::createIndenterCallString() {
  ...
  foreach (QString suffix, QStringList() << ""
                                         << ".exe"
                                         << ".bat"
                                         << ".com"
                                         << ".sh") {
    ...
    if (QFile::exists(_indenterExecutableCallString)) {
      ...
      if (QFileInfo(_indenterExecutableCallString).suffix().toLower() !=
          "php") {
        indentProcess.start("\"" + _indenterExecutableCallString + +"\" " +
                            _indenterShowHelpParameter);
      ...
      // or test if its a php file.
      QString interpreterName = "";
      QFile indenterExecutable(_indenterExecutableCallString);

      // If indenter executable file has .php as suffix, use php as default
      // interpreter
      if (QFileInfo(_indenterExecutableCallString).suffix().toLower() ==
          "php") {
      ...
      // Else try to open the file and read the shebang.
        ...
        if (firstLineOfIndenterExe.startsWith("#!")) {
          // Get the rightmost word. by splitting the string into only full
          // words.
      ...
      if (!interpreterName.isEmpty()) {
        _indenterExecutableCallString =
            interpreterName + " \"" + _indenterExecutableCallString + "\"";
        ...
        // now we know an interpreter is needed but it could not be called, so
        // inform the user.
        else {
          _errorMessageDialog->showMessage(
              tr("Interpreter needed"),
              tr("To use the selected indenter the program \"%1\" needs to be "
                 "available in the global environment. You should add an entry "
  ...
  // If even globally calling the indenter fails, try calling .com and .exe via
  // wine
  // -------------------------------------------------------------------------------
  _indenterExecutableCallString =
      "\"" + QFileInfo(_indenterDirctoryStr).absoluteFilePath() + "/" +
  ...
  foreach (QString suffix, QStringList() << ".exe"
                                         << ".com") {
    _indenterExecutableSuffix = suffix;
    if (QFile::exists(_indenterDirctoryStr + "/" + _indenterFileName +
                      suffix)) {
      QProcess wineTestProcess;
      wineTestProcess.start("wine --version");
      // if the process of wine was not callable assume that wine is not
      // installed
      if (!wineTestProcess.waitForFinished(2000)) {
        _errorMessageDialog->showMessage(
            tr("wine not installed"),
            tr("There exists only a win32 executable of the indenter and wine "
               "does not seem to be installed. Please install wine to be able "
               "to run the indenter."));
        _indenterExecutableCallString = "";
        return false;
      } else {
...
/*!
    \brief Returns a string that points to where the indenters manual can be
   found.
 */
...
/*!
    \brief Can be called to update all widgets text to the currently selected
   language.
 */
void IndentHandler::retranslateUi() {
  _indenterSelectionCombobox->setToolTip(tr(
      "<html><head><meta name=\"qrichtext\" content=\"1\" /></head><body "
      "style=\" white-space: pre-wrap; font-family:MS Shell Dlg; "
      "font-size:8.25pt; font-weight:400; font-style:normal; "
      "text-decoration:none;\"><p style=\" margin-top:0px; margin-bottom:0px; "
      "margin-left:0px; margin-right:0px; -qt-block-indent:0; "
      "text-indent:0px;\">Shows the currently chosen indenters name and lets "
      "you choose other available indenters</p></body></html>"));
  _indenterParameterHelpButton->setToolTip(
      tr("Brings you to the online manual of the currently selected indenter, "
         "where you can get further help on the possible parameters."));

  _actionLoadIndenterConfigFile->setText(
      QApplication::translate("IndentHandler", "Load Indenter Config File", 0));
  _actionLoadIndenterConfigFile->setStatusTip(QApplication::translate(
      "IndentHandler",
      "Opens a file dialog to load the original config file of the indenter.",
      0));
  _actionLoadIndenterConfigFile->setShortcut(
      QApplication::translate("IndentHandler", "Alt+O", 0));

  _actionSaveIndenterConfigFile->setText(
      QApplication::translate("IndentHandler", "Save Indenter Config File", 0));
  _actionSaveIndenterConfigFile->setStatusTip(QApplication::translate(
      "IndentHandler",
      "Opens a dialog to save the current indenter configuration to a file.",
      0));
  _actionSaveIndenterConfigFile->setShortcut(
      QApplication::translate("IndentHandler", "Alt+S", 0));

  _actionCreateShellScript->setText(QApplication::translate(
      "IndentHandler", "Create Indenter Call Shell Script", 0));
  _actionCreateShellScript->setToolTip(QApplication::translate(
      "IndentHandler",
      "Create a shell script that calls the current selected indenter for "
      "formatting an as parameter given file with the current indent settings.",
      0));
  _actionCreateShellScript->setStatusTip(QApplication::translate(
      "IndentHandler",
      "Create a shell script that calls the current selected indenter for "
      "formatting an as parameter given file with the current indent settings.",
      0));

  _actionResetIndenterParameters->setText(
      QApplication::translate("IndentHandler", "Reset indenter parameters", 0));
  _actionResetIndenterParameters->setToolTip(QApplication::translate(
      "IndentHandler", "Resets all indenter parameters to the default values.",
      0));
  _actionResetIndenterParameters->setStatusTip(QApplication::translate(
      "IndentHandler", "Resets all indenter parameters to the default values.",
      0));
}
...
QString IndentHandler::getCurrentIndenterName() {
  ...
  // Remove the supported programming languages from indenters name, which are
  // set in braces.
  if (currentIndenterName.indexOf("(") > 0) {
    // Using index-1 to also leave out the blank before the brace.
    currentIndenterName =
        currentIndenterName.left(currentIndenterName.indexOf("(") - 1);
  }
...
/*!
    \brief Shows a file open dialog to open an existing config file for the
   currently selected indenter.

    If the file was successfully opened the indent handler is called to load the
   settings and update itself.
*/
void IndentHandler::openConfigFileDialog() {
  QString configFilePath;

  configFilePath =
      QFileDialog::getOpenFileName(NULL, tr("Choose indenter config file"),
                                   getIndenterCfgFile(), "All files (*.*)");

  if (configFilePath != "") {
    // If the config file was loaded successfully, inform any who is interested
    // about it.
...
/*!
    \brief Calls the indenter config file save as dialog to save the config file
   under a chosen name.

    If the file already exists and it should be overwritten, a warning is shown
   before.
*/
void IndentHandler::saveasIndentCfgFileDialog() {
  QString fileExtensions = tr("All files") + " (*.*)";

  // QString openedSourceFileContent = openFileDialog( tr("Choose source code
  // file"), "./", fileExtensions );
  QString fileName =
      QFileDialog::getSaveFileName(this, tr("Save indent config file"),
                                   getIndenterCfgFile(), fileExtensions);
...
void IndentHandler::createIndenterCallShellScript() {
  ...
  QString fileExtensions = tr("Shell Script") + " (*." + shellScriptExtension +
                           ");;" + tr("All files") + " (*.*)";

  QString currentIndenterName = getCurrentIndenterName();
  currentIndenterName = currentIndenterName.replace(" ", "_");

  QString shellScriptFileName = QFileDialog::getSaveFileName(
      this, tr("Save shell script"),
      "call_" + currentIndenterName + "." + shellScriptExtension,
      fileExtensions);
  ...
  if (outSrcFile.open(QFile::ReadWrite | QFile::Text)) {
    QString shellScriptConfigFilename =
        QFileInfo(shellScriptFileName).baseName() + "." +
        QFileInfo(_globalConfigFilename).suffix();

    // Get the content of the shell/batch script.
    QString indenterCallShellScript =
        generateShellScript(shellScriptConfigFilename);

    // Replace placeholder for script name in script template.
    indenterCallShellScript = indenterCallShellScript.replace(
        "__INDENTERCALLSTRINGSCRIPTNAME__",
        QFileInfo(shellScriptFileName).fileName());

    outSrcFile.write(indenterCallShellScript.toLatin1());
#if !defined(Q_OS_WIN32)
    // For none Windows systems set the files executable flag
    outSrcFile.setPermissions(outSrcFile.permissions() | QFile::ExeOwner |
                              QFile::ExeUser | QFile::ExeGroup);
#endif
    outSrcFile.close();

    // Save the indenter config file to the same directory, where the shell
    // srcipt was saved to, because the script will reference it there via "./".
    if (!_globalConfigFilename.isEmpty()) {
      saveConfigFile(QFileInfo(shellScriptFileName).path() + "/" +
                         shellScriptConfigFilename,
                     getParameterString());
    }
  }
}
...
/*!
    \brief Resets all parameters to the indenters default values as they are
   specified in the uigui ini file but asks the user whether to do it really.
 */
void IndentHandler::resetIndenterParameter() {
  int messageBoxAnswer =
      QMessageBox::question(this, tr("Really reset parameters?"),
                            tr("Do you really want to reset the indenter "
...
/*!
    \brief Sets the function pointer \a _parameterChangedCallback to the given
   callback function \a paramChangedCallback.

    Is needed for use as Notepad++ plugin.
 */
void IndentHandler::setParameterChangedCallback(
    void (*paramChangedCallback)(void)) {
  _parameterChangedCallback = paramChangedCallback;
}

/*!
    \brief Emits the \a indenterSettingsChanged signal and if set executes the
   \a _parameterChangedCallback function.
...
/*!
    \brief Sets a callback function that shall be called, when the this indenter
   parameter window gets closed.
...
/*!
    \brief Is called on this indenter parameter window close and if set calls
   the function \a _windowClosedCallback.
...
/*!
    \brief Converts characters < > and & in the \a text to HTML codes &lt &gt
   and &amp.
 */
