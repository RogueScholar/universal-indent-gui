...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD reasonForCall, LPVOID lpReserved) {
  ...
  case DLL_PROCESS_ATTACH: {
    if (!qApp) {
      ...
      indentHandler->setWindowIcon(
          QIcon(QString::fromUtf8(":/mainWindow/icon2.png")));
      indentHandler->setParameterChangedCallback(NULL);
      indentHandler->setWindowClosedCallback(showUiGUI);

      // Setting UTF-8 as default 8-Bit encoding to ensure that qDebug does no
      // false string conversion.
      QTextCodec::setCodecForCStrings(QTextCodec::codecForName("UTF-8"));
      QTextCodec::setCodecForLocale(QTextCodec::codecForName("UTF-8"));
      // Force creation of an UiGuiLogger instance here, to avoid recursion with
      // SettingsPaths init function.
      UiGuiLogger::getInstance();
      qInstallMsgHandler(UiGuiLogger::messageHandler);
      UiGuiLogger::messageHandler(UiGuiInfoMsg,
                                  QString("Starting UiGUI Version %1 %2")
                                      .arg(PROGRAM_VERSION_STRING)
                                      .arg(PROGRAM_REVISION)
                                      .toLatin1());
      UiGuiLogger::messageHandler(
          UiGuiInfoMsg, QString("Running on %1")
                            .arg(UiGuiSystemInfo::getOperatingSystem())
                            .toLatin1());
    ...
    /* Fill menu names */
#ifdef UNICODE
    wcscpy(funcItem[TOGGLE_SHOW_UIGUI_INDEX]._itemName,
           _T("&Show Parameter Settings"));
    wcscpy(funcItem[EXECUTE_TEXT_INDENT_INDEX]._itemName, _T("&Indent text"));
    wcscpy(funcItem[TOGGLE_AUTO_UPDATE_INDEX]._itemName,
           _T("&Enable Text Auto Update"));
#else
    strcpy(funcItem[TOGGLE_SHOW_UIGUI_INDEX]._itemName,
           "&Show Parameter Settings");
    strcpy(funcItem[EXECUTE_TEXT_INDENT_INDEX]._itemName, "&Indent text");
    strcpy(funcItem[TOGGLE_AUTO_UPDATE_INDEX]._itemName,
           "&Enable Text Auto Update");
#endif
...
extern "C" __declspec(dllexport) void setInfo(NppData notpadPlusData) {
  ...
  /* initial dialogs */
  // universalIndentGUI_NPPDialog.init((HINSTANCE)g_hModule, nppData,
  // &pluginProp);
}
...
extern "C" __declspec(dllexport) void beNotified(SCNotification *notifyCode) {
  if (notifyCode->nmhdr.hwndFrom == nppData._nppHandle) {
    /* on this notification code you can register your plugin icon in Notepad++
     * toolbar */
    if (notifyCode->nmhdr.code == NPPN_TBMODIFICATION) {
      // g_TBWndMgr.hToolbarBmp = (HBITMAP)::LoadImage((HINSTANCE)g_hModule,
      // MAKEINTRESOURCE(IDB_TOOLBAR), IMAGE_BITMAP, 0, 0, (LR_LOADMAP3DCOLORS));
      //::SendMessage(nppData._nppHandle, NPPM_ADDTOOLBARICON,
      //(WPARAM)funcItem[TOGGLE_AUTO_UPDATE_INDEX]._cmdID, (LPARAM)&g_TBWndMgr);
    }
  }
}
...
void loadSettings(void) {
  ...
  if (PathFileExists(iniFilePath) == FALSE) {
    ::CloseHandle(::CreateFile(iniFilePath, GENERIC_READ | GENERIC_WRITE,
                               FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                               OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL));
  }

  pluginProp.iValue1 =
      ::GetPrivateProfileInt(dlgTemp, Value1, 200, iniFilePath);
  pluginProp.iValue2 =
      ::GetPrivateProfileInt(dlgTemp, Value2, 200, iniFilePath);
}
...
#ifdef UNICODE
  ::WritePrivateProfileString(dlgTemp, Value1,
                              _itow(pluginProp.iValue1, temp, 10), iniFilePath);
  ::WritePrivateProfileString(dlgTemp, Value2,
                              _itow(pluginProp.iValue2, temp, 10), iniFilePath);
#else
  ::WritePrivateProfileString(dlgTemp, Value1,
                              _itoa(pluginProp.iValue1, temp, 10), iniFilePath);
  ::WritePrivateProfileString(dlgTemp, Value2,
                              _itoa(pluginProp.iValue2, temp, 10), iniFilePath);
#endif
}
...
void toggleAutoUpdate(void) {
  ...
  UINT state = ::GetMenuState(hMenu, funcItem[TOGGLE_AUTO_UPDATE_INDEX]._cmdID,
                              MF_BYCOMMAND);

  if (state & MF_CHECKED) {
    indentHandler->setParameterChangedCallback(NULL);
    state = ::CheckMenuItem(hMenu, funcItem[TOGGLE_AUTO_UPDATE_INDEX]._cmdID,
...
void showUiGUI(void) {
  HMENU hMenu = ::GetMenu(nppData._nppHandle);
  UINT menuState = ::GetMenuState(
      hMenu, funcItem[TOGGLE_SHOW_UIGUI_INDEX]._cmdID, MF_BYCOMMAND);
  ...
  if (menuState & MF_CHECKED) {
    ...
    menuState = ::CheckMenuItem(hMenu, funcItem[TOGGLE_SHOW_UIGUI_INDEX]._cmdID,
                                MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    if (!windowIsVisible) {
      indentHandler->show();
...
void indentText() {
  ...
  //::SendMessage(nppData._nppHandle, WM_GETCURRENTSCINTILLA, 0,
  //(LPARAM)&currentEdit);

  // Get the length of the selected text plus a 0 byte ending.
  textLength =
      ::SendMessage(getCurrentHScintilla(currentEdit), SCI_GETSELTEXT, 0, 0);

  // No text has been selected to format the whole text.
  if (textLength - 1 == 0) {
    textLength = ::SendMessage(getCurrentHScintilla(currentEdit),
    ...
    // Get whole text.
    ::SendMessage(getCurrentHScintilla(currentEdit), SCI_GETTEXT, textLength,
                  (LPARAM)fullEditorText);

    // Get the first visible line and add the max visible lines to it so later
    // scrolling to the correct position is ensured.
    int firstLine = ::SendMessage(getCurrentHScintilla(currentEdit),
                                  SCI_GETFIRSTVISIBLELINE, 0, 0);
    firstLine += ::SendMessage(getCurrentHScintilla(currentEdit),
    ...
    // Set whole text.
    ::SendMessage(getCurrentHScintilla(currentEdit), SCI_SETTEXT, 0,
                  (LPARAM)indentedTextByteArray.constData());

    // Set first visible line again.
    ::SendMessage(getCurrentHScintilla(currentEdit), SCI_GOTOLINE, firstLine,
                  0);
  }
  // Format only the selected text.
    ...
    // Get selection start.
    int selectionStartPos = ::SendMessage(getCurrentHScintilla(currentEdit),
                                          SCI_GETSELECTIONSTART, 0, 0);

    // Get the selected text.
    ::SendMessage(getCurrentHScintilla(currentEdit), SCI_GETSELTEXT, 0,
                  (LPARAM)fullEditorText);
    ...
    ::SendMessage(getCurrentHScintilla(currentEdit), SCI_REPLACESEL, 0,
                  (LPARAM)indentedTextByteArray.constData());

    // Set selection again.
    ::SendMessage(getCurrentHScintilla(currentEdit), SCI_SETSEL,
                  selectionStartPos,
                  selectionStartPos + indentedTextByteArray.length());
  }
