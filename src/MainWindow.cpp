...
/*!
    ...
    The MainWindow class is responsible for generating and displaying most of
   the gui elements. Its look is set in the file "mainwindow.ui". An object for
   the indent handler is generated here and user actions are being controlled.
   Is responsible for file open dialogs and indenter selection.
 */

/*!
    \brief Constructs the main window.
 */
MainWindow::MainWindow(QString file2OpenOnStart, QWidget *parent)
    : QMainWindow(parent), _mainWindowForm(NULL), _qSciSourceCodeEditor(NULL),
      _settings(NULL), _highlighter(NULL), _textEditVScrollBar(NULL),
      _aboutDialog(NULL), _aboutDialogGraphicsView(NULL), _settingsDialog(NULL),
      _encodingActionGroup(NULL), _saveEncodedActionGroup(NULL),
      _highlighterActionGroup(NULL), _uiGuiTranslator(NULL),
      _qTTranslator(NULL), _toolBarWidget(NULL), _indentHandler(NULL),
      _updateCheckDialog(NULL), _textEditLineColumnInfoLabel(NULL) {
  ...
  connect(_toolBarWidget->pbAbout, SIGNAL(clicked()), this,
          SLOT(showAboutDialog()));
  connect(_mainWindowForm->actionAbout_UniversalIndentGUI, SIGNAL(triggered()),
          this, SLOT(showAboutDialog()));

  // Generate settings dialog box
  _settingsDialog = new UiGuiSettingsDialog(this, _settings);
  connect(_mainWindowForm->actionShowSettings, SIGNAL(triggered()),
          _settingsDialog, SLOT(showDialog()));

  // If a file that should be opened on start has been handed over to the
  // constructor exists, load it
  ...
  // Check if a newer version is available but only if the setting for that is
  // enabled and today not already a check has been done.
  if (_settings->getValueByName("CheckForUpdate").toBool() &&
      QDate::currentDate() !=
          _settings->getValueByName("LastUpdateCheck").toDate()) {
...
/*!
    \brief Initializes the main window by creating the main gui and make some
   _settings.
 */
void MainWindow::initMainWindow() {
...
#ifndef Q_OS_MAC // On Mac restoring the window state causes the screenshot no
                 // longer to work.
  ...
  _settings->registerObjectProperty(
      _mainWindowForm->loadLastOpenedFileOnStartupAction, "checked",
      "loadLastSourceCodeFileOnStartup");

  // Tell the QScintilla editor if it has to show white space.
  connect(_mainWindowForm->whiteSpaceIsVisibleAction, SIGNAL(toggled(bool)),
          this, SLOT(setWhiteSpaceVisibility(bool)));
  // Register the white space setting in the menu to the _settings object.
  _settings->registerObjectProperty(_mainWindowForm->whiteSpaceIsVisibleAction,
  ...
  connect(_mainWindowForm->actionSave_Source_File, SIGNAL(triggered()), this,
          SLOT(saveSourceFile()));
  connect(_mainWindowForm->actionExportPDF, SIGNAL(triggered()), this,
          SLOT(exportToPDF()));
  connect(_mainWindowForm->actionExportHTML, SIGNAL(triggered()), this,
          SLOT(exportToHTML()));
  connect(_mainWindowForm->actionCheck_for_update, SIGNAL(triggered()),
          _updateCheckDialog, SLOT(checkForUpdateAndShowDialog()));
  ...
  connect(_mainWindowForm->menuRecently_Opened_Files,
          SIGNAL(triggered(QAction *)), this,
          SLOT(openFileFromRecentlyOpenedList(QAction *)));
  // connect( _settings, SIGNAL(recentlyOpenedListSize(int)), this,
  // SLOT(updateRecentlyOpenedList()) );
  _settings->registerObjectSlot(this, "updateRecentlyOpenedList()",
...
void MainWindow::initToolBar() {
  ...
  _mainWindowForm->toolBar->setAllowedAreas(Qt::TopToolBarArea |
                                            Qt::BottomToolBarArea);

  // Connect the tool bar widgets to their functions.
  _settings->registerObjectProperty(
      _toolBarWidget->enableSyntaxHighlightningCheckBox, "checked",
  ...
  connect(_toolBarWidget->cbLivePreview, SIGNAL(toggled(bool)),
          _mainWindowForm->actionLive_Indent_Preview, SLOT(setChecked(bool)));
  connect(_mainWindowForm->actionLive_Indent_Preview, SIGNAL(toggled(bool)),
          _toolBarWidget->cbLivePreview, SLOT(setChecked(bool)));
}

/*!
    \brief Create and initialize the text editor component. It uses the
   QScintilla widget.
 */
void MainWindow::initTextEditor() {
  // Create the QScintilla widget and add it to the layout.
  qDebug() << "Trying to load QScintilla library. If anything fails during "
              "loading, it might be possible that"
           << " the debug and release version of QScintilla are mixed or the "
              "library cannot be found at all.";
  // Try and catch doesn't seem to catch the runtime error when starting UiGUI
  // release with QScintilla debug lib and the other way around.
  try {
    _qSciSourceCodeEditor = new QsciScintilla(this);
  } catch (...) {
    QMessageBox::critical(
        this, "Error creating QScintilla text editor component!",
        "During trying to create the text editor component, that is based on "
        "QScintilla, an error occurred. Please make sure that you have "
        "installed QScintilla and not mixed release and debug versions.");
    exit(1);
  }
  ...
  _qSciSourceCodeEditor->setBraceMatching(
      _qSciSourceCodeEditor->SloppyBraceMatch);
  ...
  bool whiteSpaceIsVisible =
      _settings->getValueByName("whiteSpaceIsVisible").toBool();
  ...
  _textEditLineColumnInfoLabel =
      new QLabel(tr("Line %1, Column %2").arg(1).arg(1));
  ...
  connect(_qSciSourceCodeEditor, SIGNAL(linesChanged()), this,
          SLOT(numberOfLinesChanged()));
  // connect( _settings, SIGNAL(tabWidth(int)), _qSciSourceCodeEditor,
  // SLOT(setTabWidth(int)) );
  _settings->registerObjectSlot(_qSciSourceCodeEditor, "setTabWidth(int)",
                                "tabWidth");
  _qSciSourceCodeEditor->setTabWidth(
      _settings->getValueByName("tabWidth").toInt());
}

/*!
    \brief Create and init the syntax _highlighter and set it to use the
   QScintilla edit component.
 */
void MainWindow::initSyntaxHighlighter() {
  // Create the _highlighter.
  _highlighter = new UiGuiHighlighter(_qSciSourceCodeEditor);

  // Connect the syntax highlighting setting in the menu to the
  // turnHighlightOnOff function.
  connect(_mainWindowForm->enableSyntaxHighlightingAction,
          SIGNAL(toggled(bool)), this, SLOT(turnHighlightOnOff(bool)));

  // Register the syntax highlighting setting in the menu to the _settings
  // object.
  _settings->registerObjectProperty(
      _mainWindowForm->enableSyntaxHighlightingAction, "checked",
      "SyntaxHighlightingEnabled");
}
...
/*!
    ...
    If the program language is defined in the _settings, the corresponding
   language file will be loaded and set for the application. If not set there,
   the system default language will be set, if a translation file for that
   language exists. Returns true, if the translation file could be loaded.
   Otherwise it returns false and uses the default language, which is English.
 */
bool MainWindow::initApplicationLanguage() {
  ...
  // If no language was set, indicated by a negative index, use the system
  // language.
  if (languageIndex < 0) {
    ...
    // If no translation file for the systems local language exist, fall back to
    // English.
  ...
  translationFileLoaded =
      _qTTranslator->load(SettingsPaths::getGlobalFilesPath() +
  ...
  translationFileLoaded =
      _uiGuiTranslator->load(SettingsPaths::getGlobalFilesPath() +
...
void MainWindow::initIndenter() {
  ...
  _indentHandler = new IndentHandler(_currentIndenterID, this,
                                     _mainWindowForm->centralwidget);
  _mainWindowForm->vboxLayout->addWidget(_indentHandler);

  // If _settings for the indenter have changed, let the main window know aboud
  // it.
  ...
  _settings->registerObjectProperty(
      _mainWindowForm->indenterParameterTooltipsEnabledAction, "checked",
      "indenterParameterTooltipsEnabled");

  // Add the indenters context menu to the mainwindows menu.
  _mainWindowForm->menuIndenter->addActions(
      _indentHandler->getIndenterMenuActions());
}

/*!
    \brief Tries to load the by \a filePath defined file and returns its content
   as QString.
...
QString MainWindow::loadFile(QString filePath) {
  ...
  if (!inSrcFile.open(QFile::ReadOnly | QFile::Text)) {
    QMessageBox::warning(NULL, tr("Error opening file"),
    ...
    int indexOfHighlighter =
        _highlighter->setLexerForExtension(_currentSourceFileExtension);
...
/*!
    \brief Calls the source file open dialog to load a source file for the
   formatting preview.

    If the file was successfully loaded the indenter will be called to generate
...
void MainWindow::openSourceFileDialog(QString fileName) {
  ...
  QString fileExtensions = tr("Supported by indenter") + " (" +
                           _indentHandler->getPossibleIndenterFileExtensions() +
                           ");;" + tr("All files") + " (*.*)";

  // QString openedSourceFileContent = openFileDialog( tr("Choose source code
  // file"), "./", fileExtensions );
  if (fileName.isEmpty()) {
    fileName = QFileDialog::getOpenFileName(this, tr("Choose source code file"),
                                            _currentSourceFile, fileExtensions);
  }
...
/*!
    \brief Calls the source file save as dialog to save a source file under a
   chosen name.
...
bool MainWindow::saveasSourceFileDialog(QAction *chosenEncodingAction) {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save source code file"), _currentSourceFile, fileExtensions);
...
/*!
    \brief Saves the currently shown source code to the last save or opened
   source file.

    If no source file has been opened, because only the static example has been
   loaded, the save as file dialog will be shown.
 */
...
/*!
    ...
    Shows a file open dialog with the title \a dialogHeaderStr starting in the
   directory \a startPath and with a file mask defined by \a fileMaskStr.
   Returns the contents of the file as QString.
 */
QString MainWindow::openFileDialog(QString dialogHeaderStr, QString startPath,
                                   QString fileMaskStr) {
  QString fileContent = "";

  QString fileName = QFileDialog::getOpenFileName(NULL, dialogHeaderStr,
                                                  startPath, fileMaskStr);
...
/*!
    ...
    Updates the text edit field, which is showing the loaded, and if preview is
   enabled formatted, source code. Reassigns the line numbers and in case of
   switch between preview and none preview keeps the text field at the same line
   number.
 */
...
/*!
    \brief Calls the selected indenter with the currently loaded source code to
   retrieve the formatted source code.

    The original loaded source code file will not be changed.
 */
void MainWindow::callIndenter() {
  QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
  _sourceFormattedContent = _indentHandler->callIndenter(
      _sourceFileContent, _currentSourceFileExtension);
  // updateSourceView();
  QApplication::restoreOverrideCursor();
}

/*!
    \brief Switches the syntax highlighting corresponding to the value \a turnOn
   either on or off.
 */
...
/*!
    \brief Is emitted whenever the text inside the source view window changes.
   Calls the indenter to format the changed source code.
 */
void MainWindow::sourceCodeChangedSlot() {
  ...
  if (_sourceFileContent.isEmpty()) {
    // Add this line feed, because AStyle has problems with a totally emtpy
    // file.
    _sourceFileContent += "\n";
    cursorPosAbsolut = 0;
    cursorPos = 0;
    ...
    cursorPosAbsolut = _qSciSourceCodeEditor->SendScintilla(
  ...
  if (_toolBarWidget->cbLivePreview->isChecked() &&
      !enteredCharacter.isNull() && enteredCharacter != 10) {
    ...
    for (cursorLine = saveCursorLine;
         cursorLine - saveCursorLine < 6 &&
         cursorLine < _qSciSourceCodeEditor->lines();
         cursorLine++) {
      text = _qSciSourceCodeEditor->text(cursorLine);
      while (cursorPos < text.count() &&
    ...
    if (!charFound) {
      ...
      for (cursorLine = saveCursorLine;
           saveCursorLine - cursorLine < 6 && cursorLine >= 0; cursorLine--) {
        text = _qSciSourceCodeEditor->text(cursorLine);
    ...
    // ...if it was not found, set the previous cursor position.
      ...
      _qSciSourceCodeEditor->setCursorPosition(saveCursorLine,
                                               saveCursorPos + 1);
    }
  }
  ...
  if (_savedSourceContent == _qSciSourceCodeEditor->text()) {
    ...
    _qSciSourceCodeEditor->setModified(
        true); // Has no effect according to QScintilla docs.
    setWindowModified(true);
  }

  // Could set cursor this way and use normal linear search in text instead of
  // columns and rows.
  //_qSciSourceCodeEditor->SendScintilla(QsciScintillaBase::SCI_SETCURRENTPOS,
  //50); _qSciSourceCodeEditor->SendScintilla(QsciScintillaBase::SCI_SETANCHOR,
  //50);
}

/*!
    \brief This slot is called whenever one of the indenter _settings are
   changed.
...
void MainWindow::updateWindowTitle() {
  this->setWindowTitle("UniversalIndentGUI " + QString(PROGRAM_VERSION_STRING) +
                       " [*]" + _currentSourceFile);
}
...
void MainWindow::exportToPDF() {
  ...
  fileName.replace(fileName.length() - fileExtension.length(),
                   fileExtension.length(), "pdf");
  fileName = QFileDialog::getSaveFileName(this, tr("Export source code file"),
...
void MainWindow::exportToHTML() {
  ...
  if (!fileName.isEmpty()) {
    ...
    // To ensure that empty lines are kept in the HTML code make this
    // replacement.
...
/*!
    \brief Loads the last opened file if this option is enabled in the
   _settings.

    If the file does not exist, the default example file is tried to be loaded.
   If even that fails a very small code example is shown. If the setting for
   opening the last file is disabled, the editor is empty on startup.
*/
void MainWindow::loadLastOpenedFile() {
  // Get setting for last opened source code file.
  _loadLastSourceCodeFileOnStartup =
      _settings->getValueByName("loadLastSourceCodeFileOnStartup").toBool();
  ...
  if (_loadLastSourceCodeFileOnStartup) {
    ...
    _currentSourceFile = _settings->getValueByName("lastSourceCodeFile")
                             .toString()
                             .split("|")
                             .first();
    ...
    // If the last opened source code file does not exist, try to load the
    // default example.cpp file.
    ...
    // If neither the example source code file exists show some small code
    // example.
    else {
  ...
  // if last opened source file should not be loaded make some default
  // _settings.
  else {
    _currentSourceFile = "untitled.cpp";
    _currentSourceFileExtension = "cpp";
  ...
  // Update the mainwindow title to show the name of the loaded source code
  // file.
  updateWindowTitle();
}

/*!
    \brief Saves the _settings for the main application to the file
   "UniversalIndentGUI.ini".

    Settings are for example last selected indenter, last loaded config file and
   so on.
*/
...
/*!
    \brief Is always called when the program is quit. Calls the saveSettings
   function before really quits.
*/
...
/*!
    ...
    All widgets that are created by the _indentHandler object and are
   responsible for indenter parameters are connected with this event filter. So
   depending on the _settings the tooltips can be enabled and disabled for these
   widgets.
 */
...
/*!
    \brief Is called at application exit and asks whether to save the source
   code file, if it has been changed.
 */
...
/*!
    \brief This slot is called whenever a language is selected in the menu. It
   tries to find the corresponding action in the languageInfoList and sets the
   language.
 */
void MainWindow::languageChanged(int languageIndex) {
  if (languageIndex < _settings->getAvailableTranslations().size()) {
    ...
    QString languageShort =
        _settings->getAvailableTranslations().at(languageIndex);
    ...
    translationFileLoaded = _uiGuiTranslator->load(
        SettingsPaths::getGlobalFilesPath() + "/translations/universalindent_" +
        languageShort);
    if (translationFileLoaded) {
      qApp->installTranslator(_uiGuiTranslator);
    }
...
/*!
    \brief Creates a menu entries in the file menu for opening and saving a file
...
void MainWindow::createEncodingMenu() {
  ...
  _encodingsList = QStringList() << "UTF-8"
                                 << "UTF-16"
                                 << "UTF-16BE"
                                 << "UTF-16LE"
                                 << "Apple Roman"
  ...
  foreach (encodingName, _encodingsList) {
    ...
    encodingAction->setStatusTip(tr("Reopen the currently opened source code "
                                    "file by using the text encoding scheme ") +
                                 encodingName);
    ...
    encodingAction->setStatusTip(tr("Save the currently opened source code "
                                    "file by using the text encoding scheme ") +
                                 encodingName);
  }

  _mainWindowForm->encodingMenu->addActions(_encodingActionGroup->actions());
  connect(_encodingActionGroup, SIGNAL(triggered(QAction *)), this,
          SLOT(encodingChanged(QAction *)));

  _mainWindowForm->saveEncodedMenu->addActions(
      _saveEncodedActionGroup->actions());
  connect(_saveEncodedActionGroup, SIGNAL(triggered(QAction *)), this,
          SLOT(saveAsOtherEncoding(QAction *)));
}

/*!
    \brief This slot calls the save dialog to save the current source file with
   another encoding.

    If the saving is successful and not aborted, the currently used encoding,
   visible in the "reopen" menu, is also changed to the new encoding.
*/
void MainWindow::saveAsOtherEncoding(QAction *chosenEncodingAction) {
  ...
  // If the file was save with another encoding, change the selected encoding in
  // the reopen menu.
  if (fileWasSaved) {
...
/*!
    \brief This slot is called whenever an encoding is selected in the settings
   menu.
*/
...
/*!
    \brief Creates a menu entry under the settings menu for all available text
   encodings.
*/
void MainWindow::createHighlighterMenu() {
  ...
  foreach (highlighterName, _highlighter->getAvailableHighlighters()) {
    ...
    highlighterAction->setStatusTip(tr("Set the syntax highlightning to ") +
                                    highlighterName);
    highlighterAction->setCheckable(true);
  }
  _mainWindowForm->highlighterMenu->addActions(
      _highlighterActionGroup->actions());
  _mainWindowForm->menuSettings->insertMenu(
      _mainWindowForm->indenterParameterTooltipsEnabledAction,
      _mainWindowForm->highlighterMenu);

  connect(_highlighterActionGroup, SIGNAL(triggered(QAction *)), _highlighter,
          SLOT(setHighlighterByAction(QAction *)));
}

/*!
    \brief Is called whenever the white space visibility is being changed in the
   menu.
 */
void MainWindow::setWhiteSpaceVisibility(bool visible) {
  if (_qSciSourceCodeEditor != NULL) {
    if (visible) {
      ...
      _qSciSourceCodeEditor->setWhitespaceVisibility(
...
/*!
    \brief This slot is called whenever the number of lines in the editor
   changes and adapts the margin for the displayed line numbers.
*/
...
void MainWindow::changeEvent(QEvent *event) {
  ...
  if (event->type() == QEvent::LanguageChange) {
    ...
    for (i = 0; i < encodingActionList.size(); i++) {
      encodingActionList.at(i)->setStatusTip(
          tr("Reopen the currently opened source code file by using the text "
             "encoding scheme ") +
          _encodingsList.at(i));
    }
    ...
    for (i = 0; i < encodingActionList.size(); i++) {
      encodingActionList.at(i)->setStatusTip(
          tr("Save the currently opened source code file by using the text "
             "encoding scheme ") +
          _encodingsList.at(i));
    }
    ...
    i = 0;
    foreach (QString highlighterName,
             _highlighter->getAvailableHighlighters()) {
...
void MainWindow::updateRecentlyOpenedList() {
  QString fileName;
  QString filePath;
  QStringList recentlyOpenedList =
      _settings->getValueByName("lastSourceCodeFile").toString().split("|");
  QList<QAction *> recentlyOpenedActionList =
  ...
  // If it is in the list of recently opened files and not at the first
  // position, move it to the first pos.
  if (indexOfCurrentFile > 0) {
    recentlyOpenedList.move(indexOfCurrentFile, 0);
    recentlyOpenedActionList.move(indexOfCurrentFile, 0);
  }
  // Put the current file at the first position if it not already is and is not
  // empty.
    ...
    QAction *recentlyOpenedAction =
        new QAction(QFileInfo(_currentSourceFile).fileName(),
                    _mainWindowForm->menuRecently_Opened_Files);
  ...
  int recentlyOpenedListMaxSize =
      _settings->getValueByName("recentlyOpenedListSize").toInt();

  // Loop for each filepath in the recently opened list, remove non existing
  // files and loop only as long as maximum allowed list entries are set.
  for (int i = 0;
       i < recentlyOpenedList.size() && i < recentlyOpenedListMaxSize;) {
    ...
    // else if its not already in the menu, add it to the menu.
      ...
      if (i >= recentlyOpenedActionList.size() - 2) {
        QAction *recentlyOpenedAction = new QAction(
            fileInfo.fileName(), _mainWindowForm->menuRecently_Opened_Files);
        recentlyOpenedAction->setStatusTip(filePath);
        recentlyOpenedActionList.insert(recentlyOpenedActionList.size() - 2,
  ...
  while (recentlyOpenedList.size() > recentlyOpenedListMaxSize) {
    ...
    QAction *action =
        recentlyOpenedActionList.takeAt(recentlyOpenedActionList.size() - 3);
    delete action;
  }

  // Add all actions to the menu.
  _mainWindowForm->menuRecently_Opened_Files->addActions(
      recentlyOpenedActionList);
  _mainWindowForm->menuRecently_Opened_Files->addActions(
      recentlyOpenedActionList);
...
/*!
    \brief This slot is called if an entry from the list of recently opened
   files is being selected.
 */
void MainWindow::openFileFromRecentlyOpenedList(QAction *recentlyOpenedAction) {
  // If the selected action from the recently opened list menu is the clear
  // action call the slot to clear the list and then leave.
  if (recentlyOpenedAction ==
      _mainWindowForm->actionClear_Recently_Opened_List) {
  ...
  int indexOfSelectedFile =
      _mainWindowForm->menuRecently_Opened_Files->actions().indexOf(
          recentlyOpenedAction);
  QStringList recentlyOpenedList =
      _settings->getValueByName("lastSourceCodeFile").toString().split("|");
  ...
  // If it does not exist, show a warning message and update the list of
  // recently opened files.
  else {
    QMessageBox::warning(NULL, tr("File no longer exists"),
                         tr("The file %1 in the list of recently opened files "
                            "does no longer exist."));
    // The function updateRecentlyOpenedList() has to be called via a singleShot
    // so it is executed after this function (openFileFromRecentlyOpenedList)
    // has already been left. This has to be done because a Qt3Support function
    // tries to emit a signal based on the existing actions and deleting any of
    // these actions in updateRecentlyOpenedList() causes an error.
    QTimer::singleShot(0, this, SLOT(updateRecentlyOpenedList()));
  }
}

/*!
    \brief If the dragged in object contains urls/paths to a file, accept the
   drag.
 */
...
/*!
    \brief If the dropped in object contains urls/paths to a file, open that
   file.
 */
...
void MainWindow::showAboutDialog() {
  ...
  // QPixmap::grabWindow(QApplication::desktop()->screen()->winId()); qDebug("in
  // main pixmap width %d, numScreens = %d", originalPixmap.size().width(),
...
/*!
    \brief Sets the label in the status bar to show the \a line and \a column
   number.
*/
void MainWindow::setStatusBarCursorPosInfo(int line, int column) {
  _textEditLineColumnInfoLabel->setText(
      tr("Line %1, Column %2").arg(line + 1).arg(column + 1));
}
